...
const pass_data pass_data_kzaw =
{
  ...
  0, /* todo_flags_start */
  TODO_cleanup_cfg, /* todo_flags_finish - ensure finalize is called */
};
...
public:
  ...
  // Add finalize method to process all clone groups at the end
  unsigned int finalize () final override;
...
private:
  ...
  // New method to analyze all clone groups
  void analyze_all_clone_groups();
};
...
pass_kzaw::execute(function *fun)
{
  ...
  // Provide immediate analysis feedback for this variant if there are enough variants
  if (group.size() >= 2) {
    if (dump_file) {
      fprintf(dump_file, "Incremental analysis of clones for function: %s\n", base_name.c_str());
    }
    ...
    // If no default variant found, use the first one as reference
    if (!found_default) {
      default_idx = 0;
    }
    
    const function_info &default_info = group[default_idx];
    
    // Compare this variant with the reference if it's not the reference itself
    if (variant != default_info.variant) {
      bool are_same = compare_functions(default_info.stmts, info.stmts);
      
      if (dump_file) {
        fprintf(dump_file, "Variant %s%s is %s to %s%s\n", 
                base_name.c_str(), variant.c_str(),
                are_same ? "similar" : "different",
...
// Method to analyze all clone groups at the end of the pass
void
pass_kzaw::analyze_all_clone_groups()
{
  if (dump_file) {
    fprintf(dump_file, "\nFinal analysis of all clone groups:\n");
    fprintf(dump_file, "================================\n");
  }
  
  // Process each clone group
  for (auto &group_pair : clone_groups) {
    const std::string &base_name = group_pair.first;
    std::vector<function_info> &group = group_pair.second;
    
    if (group.size() < 2) {
      // Not enough variants to make a decision
      if (dump_file) {
        fprintf(dump_file, "NOPRUNE: %s (only %zu variants found)\n", 
                base_name.c_str(), group.size());
      }
      continue;
    }
    
    if (dump_file) {
      fprintf(dump_file, "Analyzing clone group: %s (%zu variants)\n", 
              base_name.c_str(), group.size());
    }
    
    // Find the default variant or use the first one as reference
    size_t default_idx = 0;
    bool found_default = false;
    for (size_t i = 0; i < group.size(); i++) {
      if (group[i].variant == ".default") {
    ...
    // If no default variant found, use the first one as reference
    if (!found_default && dump_file) {
      fprintf(dump_file, "No default variant found for %s, using first variant as reference\n", 
              base_name.c_str());
    }
    ...
    for (size_t i = 0; i < group.size(); i++) {
      ...
      const function_info &variant_info = group[i];
      ...
      if (!are_same) {
        ...
        if (dump_file) {
          fprintf(dump_file, "NOPRUNE: %s%s (differs from reference)\n", 
                  base_name.c_str(), variant_info.variant.c_str());
        }
      } else {
        // Print the pruning decision for this specific variant
        if (dump_file) {
          fprintf(dump_file, "PRUNE: %s%s (identical to reference)\n", 
                  base_name.c_str(), variant_info.variant.c_str());
        }
      }
    }
    
    // Print the overall pruning decision for the base function
    print_prune_decision(base_name, all_same);
  }
}

// Finalize method to process all clone groups at the end
unsigned int
pass_kzaw::finalize()
{
  // Process all collected clone groups
  analyze_all_clone_groups();
  
  // Clear all clone groups after final analysis
  clone_groups.clear();
